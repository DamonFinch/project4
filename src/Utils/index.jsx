import abi from '../abis/src/contracts/Contract.sol/Contract.json'
...
const buyNFTFromServer = async (tokenId, amount) => {
  try {
    if (!ethereum) return alert('Please install Metamask')
    const connectedAccount = getGlobalState('connectedAccount')
    const contract = getEtheriumContract()

    await contract.buyFromServer(tokenId, amount)
...
const loadMyNfts = async() => {
  try {
    ...
    setGlobalState('myNfts', structuredNfts(nfts))
  } catch (error) {
    reportError(error)
  }
}
